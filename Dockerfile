...
RUN mkdir -p /opt && \
	...
	cd /work/pkgsrc/cross/ppc-morphos-gcc && \
	printf "ACCEPTABLE_LICENSES+= morphos-sdk-license\nACCEPTABLE_LICENSES+= lha-license\n" >> /opt/etc/mk.conf && \
	sed -e 's|\${PREFIX}/gg|\${PREFIX}/ppc-morphos|' -i /work/pkgsrc/cross/ppc-morphos-gcc/Makefile && \
	sed -e 's|gg/|ppc-morphos/|' -i /work/pkgsrc/cross/ppc-morphos-gcc/PLIST && \
